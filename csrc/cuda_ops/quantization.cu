...
__global__ void per_token_group_quant_8bit_kernel(
  ...
  for (int32_t i = lane_id; i < num_vec_elems; i += 16) {
    T val = group_input[i];
    float abs_val = fabsf(static_cast<float>(val));
  ...
  for (int32_t i = lane_id; i < num_vec_elems; i += 16) {
    T val = group_input[i];
    float q_val = fminf(fmaxf(static_cast<float>(val) / y_s, min_8bit), max_8bit);
    group_output[i] = DST_DTYPE(q_val);
  }
}
...
void sgl_per_token_group_quant_8bit(
  ...
  TORCH_CHECK(input.numel() % group_size == 0, "input.numel() must be divisible by group_size");
  TORCH_CHECK(output_s.dim() == 2, "output_s must be 2D");
...
#define LAUNCH_KERNEL(T, DST_DTYPE)                                                               \
  do {                                                                                            \
    ...
    if (is_column_major) {                                                                        \
      ...
      TORCH_CHECK(!scale_ue8m0, "scale_ue8m0 requires column-major scales");                      \
  ...
  if (input.scalar_type() == at::ScalarType::BFloat16) {
    if (dst_type == at::ScalarType::Float8_e4m3fn) {
      LAUNCH_KERNEL(at::BFloat16, __nv_fp8_e4m3);
    } else if (dst_type == at::ScalarType::Char) {
      LAUNCH_KERNEL(at::BFloat16, int8_t);
    } else {
      TORCH_CHECK(false, "Unsupported dst dtype");
    }
  } else {

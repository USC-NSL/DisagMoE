...
struct Metadata {
    ...
    void get_expert_batch_sizes_cuda(int n_expert, const std::vector<int> &inner_exp_rank, torch::Tensor tensor_cuda, cudaStream_t stream) {
        AUTO_TX_RANGE;
        ASSERT(n_expert > 0);
        ...
        CUDACHECK(cudaMemcpyAsync(tensor_cuda.data_ptr(), batches, m * sizeof(int64_t), cudaMemcpyHostToDevice, stream));
    }

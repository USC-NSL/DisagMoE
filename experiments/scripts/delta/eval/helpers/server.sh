#!/usr/bin/bash
# lib/server.sh — DisagMoE server lifecycle helpers
...
#   benchmark/server.py in the background (nohup). Sets global SERVER_PID.
#   MEM_FRAC is read at call time, so OOM retries with a lower value work.
launch_server() {
...
#   Polls log until Flask is up or timeout (SERVER_READY_TIMEOUT) is reached.
...
#   Returns 0 if the log contains any CUDA/torch out-of-memory indication.
is_oom() {
    local server_log="$1"
    grep -qi \

...
@torch.inference_mode()
def benchmark_torch_matmul(hidden_size, intermediate_size, num_experts, label):
    ...
    # Allocate FP8 weights, then dequantize to BF16 for torch.matmul baseline
    BCs_fp8, Ds_fp8 = alloc_expert_weights(hidden_size, intermediate_size, num_experts, device, dtype=DTYPE_FP8)
    BCs = BCs_fp8.to(ACCUM_DTYPE).contiguous()
    Ds = Ds_fp8.to(ACCUM_DTYPE).contiguous()

    # --- Sequential CUDA graph case (torch.matmul) ---
    ...
    for n_rows in row_sizes:
        ...
        results_torch_matmul.append(np.mean(results_this_batch_size))
            
    torch.cuda.empty_cache()

    # No plotting here; just return results
    return row_sizes, results_torch_matmul
...
for idx, (hidden_size, intermediate_size, num_experts, label, model) in enumerate(zip(hidden_sizes, intermediate_sizes, num_experts_list, labels, models)):
    row_sizes, torch_means = benchmark_torch_matmul(hidden_size, intermediate_size, num_experts, label)
    row_sizes_sgl, sgl_means = benchmark_sgl_fp8_kernel(hidden_size, intermediate_size, num_experts, label)
    assert np.array_equal(row_sizes, row_sizes_sgl), "Row sizes mismatch between benchmarks"
    x = np.arange(len(row_sizes))
    width = 0.35
    ax = axes[idx]
    ax.bar(x - 0.5*width, torch_means, width=width, label="Torch matmul (graph)")
    ...
    time_series_torch.append(np.array(torch_means, dtype=np.float64))
...
# Plot per model with consistent color; two methods: Torch vs SGL FP8
for flops_g, t_torch, t_sgl, name, color in zip(
    flops_series_per_model,
    time_series_torch,
    time_series_sgl,
    model_names,
    ...
    ax2.plot(flops_g, t_torch, marker='o', linestyle='-', color=color, label=f"{name} - Torch matmul")
...
ax2.set_title("Time vs Estimated FLOPs (Torch matmul vs SGL FP8 kernel)")

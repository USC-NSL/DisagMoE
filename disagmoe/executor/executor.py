...
class CUDAGraphAttnExecutor:
    ...
    def capture(self):
        ...
        for layer_id in self.model_config.layer_ids:
            for graph, graph_batch_size in zip(self.graphs[layer_id], self.graph_batch_sizes):
                ...
                self.cuda_graph_preprocess(batch.data, batch.seq_lens_tensor, attn_meta)
        ...
        torch.cuda.synchronize()
        get_logger().info(f"cuda graph captured in {end_time - start_time:.2f} seconds")
        self.test_graph()
    ...
    def test_graph(self):
        ...
        torch.cuda.synchronize()
        end_time = time.perf_counter()
        get_logger().info(f"cuda graph test passed in {end_time - start_time:.2f} seconds")
